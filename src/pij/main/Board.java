		...
		addMouseListener(new MouseListener() {
			...
			public void mouseReleased(MouseEvent me) {
				if (me.getX() > 750 || me.getY() > 750) {
					...
					if (tileArr[col][row].letter < 53) {// tileArr[col][row].letter == ' ' || tileArr[col][row].letter
														// >= '0' && tileArr[col][row].letter <= '4' ){
	...
	private void render() {
		...
		for (int row = 0; row < tileArr.length; row++) {
			for (int col = 0; col < tileArr[0].length; col++) {
				...
				// canvas.getGraphics().drawImage(tileArr[row][col].image , col*50, row*50,
				// null);// (blank.icon);
			}
		}
